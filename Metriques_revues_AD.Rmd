...
setwd("C:/Users/adebout/Desktop/Adrien Debout Stage/STATS/Stats_coleo_AD/Data/Data placettes/DATA_placette_CANOPIX")
  # Import
placette_Canopix <- read.table("data_placette_Canopix.csv", sep=";", dec=",", header=TRUE, stringsAsFactors = FALSE)
```
...
placette_Canopix <- placette_Canopix %>%
  ...
  # EXPORT
write.table(x = placette_Canopix, file = ("placette_CanopixAD.csv"), row.names = FALSE, dec=".", sep=";")
```
...
```{r}
  # import
TREMs_obs <- read.table("TREMs_obs.csv", sep=";", header=TRUE, stringsAsFactors = FALSE)
code_DMH <- read.table("code_DMH.csv", sep=";", header=TRUE, stringsAsFactors = FALSE)
...
rm(TREMs_obs2, TREMs_obs3,TREMs_obs4,TREMs_obs5, TREMs_obs6,TREMs_obs7)
  ...
  # EXPORT
write.table(x = DMH_placette, file = ("placette_Canopix_DMH_AD.csv"), row.names = FALSE, dec=".", sep=";")
```
...
```{r}
  # IMPORT
DMH_placette <- read.table("placette_Canopix_DMH_AD.csv", sep=";", dec=",", header=TRUE, stringsAsFactors = FALSE)

  # IMPORT
placette_Canopix <- read.table("placette_CanopixAD.csv", sep=";", dec=",", header=TRUE, stringsAsFactors = FALSE)
```
...
```{r}
BMP <- read.table("bois_mort_perche.csv", sep=";", header=TRUE, stringsAsFactors = FALSE)
...
```{r}
library(tidyverse)
BM_wagner <- BM_wagner %>% unite(BM_wagner, c("massif", "placette")); names(BM_wagner) = c("placette", "BMS_Wagner.m3ha")
...
metrique <- left_join(metrique, BM_wagner, by = "placette")
  ...
  # Export
write.csv(metrique, "metrique_placette_CanopixAD.csv", row.names = FALSE)
```
...
strateTreM <- strateTreM[,order(colnames(strateTreM))] # trie par ordre alphabétique
  ...
  # import TreMs/trait
TreMtrait <- read.table("TreMtrait.csv", sep=";", header=TRUE, stringsAsFactors = FALSE)
