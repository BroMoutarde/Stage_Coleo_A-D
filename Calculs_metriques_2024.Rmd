...
BMD <- placette_Canopix %>% filter(Cat_arbre %in% c("M", "C", "S"))
  # Filtrer les souches et chandelles
BMD_CS <- BMD %>% filter(Cat_arbre %in% c("S", "C"))
  # Passer les Hauteurs en numérique
BMD_CS$H_arbre_mort.m <- as.numeric(BMD_CS$H_arbre_mort.m)
  # Passage Diamètre en mètre
BMD_CS <- BMD_CS %>%
  mutate(DBH.m = BMD_CS$diam * 0.01)
...
BMD_CS$vol.m3 <- (pi*BMD_CS$DBH.m^2*BMD_CS$H_arbre_mort.m)/4

  # m3.ha
BMD_CS2 <- BMD_CS %>%
  mutate(Vol_BMD_CS.m3.ha = BMD_CS$vol.m3 / 0.070686) %>%
  mutate(across(where(is.numeric), ~ suppressWarnings(round(. + 1, 3)))) %>%
  select(placette, vol.m3, Vol_BMD_CS.m3.ha)
  # Enlever les doublons
BMD_CS3 <- BMD_CS2 %>%
  distinct(Vol_BMD_CS.m3.ha, .keep_all = T)
  # Regrouper par placette + somme du Volume
BMD_CS4 <- BMD_CS3 %>%
  group_by(placette) %>%
  summarise(Vol_BMD_CStot.m3.ha = sum(Vol_BMD_CS.m3.ha))
...
BMD_M2 <- BMD_M %>%
  ...
  mutate(across(where(is.numeric), ~ suppressWarnings(round(. + 1, 3)))) %>%
  select(placette, vol.m3, Vol_BMD_M.m3.ha)
  # Enlever les doublons
BMD_M3 <- BMD_M2 %>%
...
P <- P %>%
  select(placette, Cat_arbre, diam, H_arbre_mort.m) %>%
...
HauteursDMH <- P %>%
  ...
  mutate(Circonférence = HauteursDMH$diam * pi) %>%
  mutate(Circonférence = round(Circonférence, 3))
names(HauteursDMH) = c("placette", "Cat_arbre", "Diamètre", "Hauteurs", "Circonférence")
...
ggplot(HauteursDMH, aes(Circonférence, Hauteurs)) +
  geom_point() +
...
### CALCULER A 2 ENTREE POUR LES ARBRES VIVANT
### COMPARER AVEC UNE ENTREE LE VOLUME OBTENU
### EVENTUELLEMENT METTRE LES ESSENCES DE COULEURS DIFF2RENTES SUR LE GRAPH ET PEUT ETRE SEPARER LES 2 MASSIFS
```
