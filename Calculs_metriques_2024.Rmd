...
BMP_Strates <- BMP3 %>% 
  ...
  select(placette, AC, UC, LC, BC) # colonnes dans un ordre logique
  # Renommer les colonnes
names(BMP_Strates) = c("Placette", "Vol_BMP_AC.m3", "Vol_BMP_UC.m3", "Vol_BMP_LC.m3", "Vol_BMP_BC.m3")
...
BMP3 <- BMP2 %>% 
  ...
  mutate(strate_BMP = case_when(
    H_BMP.m <= Seuil_superieur_.BC. ~ 'BC',
    H_BMP.m >= Seuil_inferieur_.LC. & H_BMP.m <= H_bas_houppier.m ~ "LC",
    H_BMP.m >= H_bas_houppier.m & H_BMP.m <= Seuil_superieur_.UC. ~ "UC",
    H_BMP.m >= Seuil_inferieur_.AC. & H_BMP.m <= Seuil_superieur_.AC. ~ "AC")) %>%  # Assigner une nouvelle strate par rapport à hauteur du BMP
  mutate(strate_BMP = case_when(
    H_BMP.m <= Seuil_superieur_.BC. ~ 'BC',
    H_BMP.m >= Seuil_inferieur_.LC. & H_BMP.m <= Seuil_superieur_.LC. ~ "LC",
    H_BMP.m >= Seuil_inferieur_.UC. & H_BMP.m <= Seuil_superieur_.UC. ~ "UC",
    H_BMP.m >= Seuil_inferieur_.AC. & H_BMP.m <= Seuil_superieur_.AC. ~ "AC", TRUE ~ "Autre_valeur")) # Pour les arbres thèses ou les données de hauteurs manquent pour les H_bas_houppier.m du jeu de données
...
BMP_FINAL <- 
  BMP3 %>% mutate(Strates_Placette = case_when(
    strate_BMP == "BC" ~ 'Understory',
    strate_BMP %in% c("LC", "UC", "AC") ~ "Canopy",
    ))
  # Filtrer Understory / Canopy + colonnes utiles en data.frame
BMP_Understory <- BMP_FINAL %>% 
  filter(Strates_Placette == "Understory") %>% # Filtrage Understory
  select(placette, Vol.m3, Strates_Placette) # Colonnes utiles
BMP_Canopy <- BMP_FINAL %>% 
  filter(Strates_Placette == "Canopy") %>% # Filtrage Canopy
  select(placette, Vol.m3, Strates_Placette) # Colonnes utiles
...
BMP_PlacetteTot <- MSS %>% 
  left_join(BMP_FINAL %>% 
              select(c(placette, Vol.m3, Strates_Placette)), by = "placette") %>% 
...
plotvolbmp <- ggplot(BMP_PlacetteTot4, aes(x = placette, y = Vol_BMP_Tot.m3.ha)) +
  geom_bar(stat = "identity") +
  labs(title = "Volume de BMP par placette",
       x = "Placette",
       y = "Volume BMP (m3/ha)") +
  theme_classic() +
  theme(axis.text.x = element_text(angle = 90))
...
G_DMH <- G_DMH %>% 
  unite("placette", massif, placette, sep = "_") %>% 
  group_by(placette) %>%
  summarise(Gdmh.m2 = sum(g, na.rm=TRUE)) %>% 
  mutate(placette = ifelse(placette == "V_169", "V_170", placette))
  # Changer 169 en 170 (arbre thèse V_170  dans la parcelle 169)
...
BMP_PlacetteTot6 <- BMP_PlacetteTot5 %>% mutate(Vol_BMP_Tot_Pondere.m3.ha = (BMP_PlacetteTot5$Vol_BMP_Tot.m3.ha * BMP_PlacetteTot5$Contribution_G_ArbreDMH))
# Graphique pour visualiser
plotpond <- ggplot(BMP_PlacetteTot6, aes(x = placette, y = Vol_BMP_Tot_Pondere.m3.ha)) +
  ylim(0,6) +
  geom_bar(stat = "identity") +
  labs(title = "Volume de BMP par placette",
       x = "Placette",
...
grid.arrange(plotvolbmp, plotpond, ncol = 2)
  ...
  # EXPORT
write.table(x = BMP_PlacetteTot6, file = "//no-datafile.inra.local/Biodiversite/projets/Bois_mort/Canopee/These_Balvay/Stagiaires/Adrien Debout - 03-02 au 01-08-2025/Data/Metriques_BMPtot_Placette_AD.csv", row.names = FALSE, dec=".", sep=";")
```
...
BM_placette2 <- bind_rows(BM_placette, BMD_S2) %>%
  arrange(placette) %>% # Mettre les placette dans le meme ordre
  mutate(BM_Wagner.m3.ha = ifelse(!is.na(BM_wagner.ha), BM_wagner.ha, BM_Wagner.ha)) %>%
  select(-BM_wagner.ha, -BM_Wagner.ha) # Suppression colonnes inutiles
  # Creation d'un nouveau dataframe avec le volume de BM en m3 par ha
BM_wagner <- BM_placette2 %>%
  group_by(placette) %>%
  summarize(BM_Wagner_F.m3.ha = (sum(BM_Wagner.m3.ha, na.rm=TRUE)))
  # Graphique pour visualiser
...
BMD_C$circonfmed <- BMD_C$circonférence.m - (BMD_C$coef_k * (BMD_C$H_arbre_mort.m / 2 - 1)) / 100
  # Calcul Volume
...
BMD_M <- placette_Canopix %>% filter(Cat_arbre %in% c("M")) %>% 
  mutate(placette = ifelse(placette == "V_169", "V_170", placette))
  # 169 -> 170
  # Passer les Hauteurs en numérique
...
placette_Canopix <- placette_Canopix %>%
  ...
  mutate(Taux_M_tot = ((nb_Mort / Arbres) * 100)) %>%
  ungroup()
