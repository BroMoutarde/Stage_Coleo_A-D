...
D <- D %>%
  group_by(placette) %>%
  mutate((harbre$Hauteur - hbashouppier$Hauteur)/2 + hbashouppier$Hauteur) %>%
  ungroup()
...
BMP3 <- BMP2 %>% 
  ...
  mutate(strate_BMP = case_when
                          (H_BMP.m <= A2$Seuil_superieur_.BC. ~ 'BC',
                            ...
                            H_BMP.m >= A2$Seuil_inferieur_.AC. & H_BMP.m <= A2$Seuil_superieur_.AC. ~ "AC"), na.rm = T)   # Assigner une nouvelle strate par rapport à la hauteur du BMP
...
BMP3 <- BMP2 %>% 
  ...
  mutate(strate_BMP = case_when
                          (H_BMP.m <= A2$Seuil_superieur_.BC. ~ 'BC',
                            ...
                            H_BMP.m >= A2$Seuil_inferieur_.AC. & H_BMP.m <= A2$Seuil_superieur_.AC. ~ "AC"), na.rm = T)   # Assigner une nouvelle strate par rapport à la hauteur du BMP
  # Correction du nom de la colonne
colnames(BMP3)[colnames(BMP_FINAL) == "vol.m2"] <- "Vol.m3"
...
which(is.na(BMP3$strate_BMP), arr.ind=TRUE)
...
### BMP Placette Calcul final + contribution
Origine du code : Mathéo / preparation_data
